import {
	buildAudioSettingsSnapshot,
	buildWindowId,
	fingerprintAudioSettings,
	measureAudioSamples,
} from "$lib/domain/audio-state";
import {
	transitionLoadState,
	type LoadStateSnapshot,
} from "$lib/domain/load-state";
import { nslcForStation, type StationId } from "$lib/domain/station";
import {
	isArchiveProvider,
	providerLabel,
	type ProviderId,
} from "$lib/domain/provider-id";
import type {
	AudioPlayer,
	AudioRenderer,
	AudioWindowSource,
...
export async function loadAudioWindow(
	command: LoadAudioWindowCommand,
): Promise<LoadedAudioWindow> {
	const window = await command.source.getAudioWindow(command.request);
	window.metrics ??= measureAudioSamples(
		window.samples,
...
export async function playAudioWindow(command: {
	...
	await command.player.play(
		command.window,
		command.soundMode,
		command.compression,
		command.listeningFocus,
	);
}
...
export async function playPreparedAudioWindow(command: {
	...
	await command.player.playPrepared(
		command.samples,
		command.renderedSampleRate,
		command.soundMode,
		command.compression,
		command.listeningFocus,
	);
...
export function makeExportMetadata(input: ExportMetadataInput) {
	return {
		...
		processing: [
			"polyphase resample to playback duration",
			"mean removal",
...
export async function exportAudioWindow(command: {
	...
	const wavFile = await command.renderer.renderWavFile(
		command.window,
		command.soundMode,
		command.compression,
		command.listeningFocus,
...
export function compareAudioSettings(
	...
	const listeningFocusChanged =
		current.listeningFocus !== loaded.listeningFocus;
	...
	const playbackSecondsChanged =
		current.playbackSeconds !== loaded.playbackSeconds;
...
export function advanceLoadState(
	...
	result:
		| { ok: true; requestedChannel: string; actualChannel: string }
