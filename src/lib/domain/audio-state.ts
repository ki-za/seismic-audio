...
export function fingerprintAudioSettings(snapshot: AudioSettingsSnapshot): string {
	return stableFingerprint(snapshot);
}

export function measureAudioSamples(samples: ArrayLike<number>, sampleRate: number): AudioMetrics {
	let sum = 0;
	let squares = 0;
	let peak = 0;
	let crossings = 0;
...
export function isStale(selectedRequestKey: string, loadedRequestKey: string | null): boolean {
