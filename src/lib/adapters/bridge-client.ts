...
const bridgeSocket =
	import.meta.env.VITE_BRIDGE_WS ?? bridgeBase.replace(/^http/, "ws");
...
export async function getAudioWindow(
	options: AudioWindowRequest,
): Promise<AudioWindow> {
	...
	try {
		const path =
			options.source === "raspberryshake"
				? "/raspberryshake/window"
				: "/window";
		...
		if (window.samples.length === 0) {
			throw appError({
				...
				message:
					"The bridge answered, but it has no samples to compress into audio.",
				recovery:
					"Wait for synthetic or UDP data to arrive, then press Begin listening again.",
			});
		}
		return window;
	} catch (error) {
		...
		throw unknownError(error, {
			...
			message:
				"The browser could not fetch the selected time window from the TypeScript bridge.",
			recovery:
				"Check that the bridge terminal is still running and that localhost:8787/window responds.",
		});
	}
}
