...
export const AUTO_CHANNELS = ['EHZ', 'SHZ', 'EHE', 'EHN', 'HHE', 'HHN', 'HHZ', 'HDF', 'ENZ', 'ENE', 'ENN'] as const;

export type RaspberryShakeStation = (typeof RASPBERRY_SHAKE_STATIONS)[number]['station'];
...
export async function loadRaspberryShakeTrace(options: {
	...
	if (!isKnownStation(options.station)) throw new Error(`Unknown Raspberry Shake station: ${options.station}`);

	const station = RASPBERRY_SHAKE_STATIONS.find((entry) => entry.station === options.station)!;
	...
	for (const channel of channels) {
		try {
			const query = new sp.fdsndataselect.DataSelectQuery('data.raspberryshake.org');
			query
				.protocol('https')
			...
			lastError = error;
			attemptedChannels.push({ channel, status: 'error', error: error instanceof Error ? error.message : String(error) });
		}
	}
...
function extractSamples(seis: unknown): Float32Array {
	...
	const segments = candidate.segments ?? candidate.segmentArray ?? candidate._segmentArray ?? [];
	const total = segments.reduce((sum, segment) => sum + (segment.y?.length ?? 0), 0);
