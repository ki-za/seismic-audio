...
async function main() {
	...
	const localRequests = [
		{ windowSeconds: 900, playbackSeconds: 10, channel: status.channels[0] ?? undefined },
	...
	for (const req of localRequests) {
		...
		console.log(`  Request: channel=${req.channel ?? '(none, defaults to first)'} windowSeconds=${req.windowSeconds} playbackSeconds=${req.playbackSeconds}`);
		console.log(`  Response: channel="${win.channel}" source=${win.source ?? '?'} station=${win.station ?? '?'}`);
		console.log(`  UI activeChannel="${activeChannelUI}"`);
		console.log(`  Match: ${match ? '✓ YES' : '✗ MISMATCH → LoadState would show "fallback"'}`);
		if (win.metadata) {
	...
	for (const stationId of STATIONS) {
		...
		try {
			...
			console.log(`  Response network.station.loc:    ${win.network ?? '?'}.${win.station ?? '?'}.${win.location ?? '?'}`);
			console.log(`  UI activeChannel (channelHint):  "${activeChannelUI}"`);
			console.log(`  Match: ${match ? '✓ YES' : '✗ MISMATCH → LoadState would show "fallback"'}`);
			console.log(`  Reason: UI sends NSLC "${activeChannelUI}" but bridge returns short code "${win.channel}"`);
	...
	console.log();
	console.log('  The activeChannel derivation in +page.svelte uses the station\'s');
	console.log('  channelHint, which is a full NSLC string like "AM.RD432.00.EHZ".');
	console.log();
	console.log('  The server returns window.channel from the actual seismogram,');
	console.log('  which is a short channel code like "EHZ".');
	console.log();
	console.log('  Result: requestedChannel !== actualChannel on every archive load,');
	...
	console.log('  2. Change the server to return the full NSLC for archive loads');
	...
	console.log('  4. Remove the fallback distinction entirely if it adds no value');
	console.log();
}
