...
import { nslcForStation, nslcToChannelHint, parseChannelHint, channelHintForStation, type NSLC } from '../src/lib/domain/station';
import { transitionLoadState, initialLoadState, loadStateLabel, type LoadStateSnapshot } from '../src/lib/domain/load-state';
import { selectProvider, compareAudioSettings, advanceLoadState, getStationNSLC } from '../src/lib/application/seismic-audio-session';
...
assert('RD432 → raspberryshake', providerFromStationId('RD432') === 'raspberryshake');
assert('label bridge',         providerLabel('bridge') === 'Local Bridge');
assert('label raspberryshake', providerLabel('raspberryshake') === 'Raspberry Shake Archive');
assert('isArchive bridge',     isArchiveProvider('bridge') === false);
assert('isArchive raspberryshake', isArchiveProvider('raspberryshake') === true);
...
const hint = nslcToChannelHint({ network: 'AM', station: 'RD432', location: '00', channel: 'EHZ' });
...
assert('channelHintForStation RD432', channelHintForStation('RD432') === 'AM.RD432.00.EHZ');
...
const loaded = transitionLoadState(loading, { kind: 'LOAD_SUCCEEDED', requestedChannel: 'EHZ', actualChannel: 'EHZ' });
assert('loading → loaded (exact match)', loaded.state === 'loaded');

const fallback = transitionLoadState(loading, { kind: 'LOAD_SUCCEEDED', requestedChannel: 'EHZ', actualChannel: 'SHZ' });
assert('loading → fallback (different channel)', fallback.state === 'fallback');

const failed = transitionLoadState(loading, { kind: 'LOAD_FAILED', error: 'timeout' });
assert('loading → failed', failed.state === 'failed' && failed.error === 'timeout');
...
assert('soundMode change detected', differentMode.anyChanged === true && differentMode.soundModeChanged);

const differentComp = compareAudioSettings({
	...base,
	compression: { ...base.compression, ratio: 8 }
}, base);
assert('compression change detected', differentComp.anyChanged === true && differentComp.compressionChanged);
...
const multiChange = compareAudioSettings({
	...
	compression: { thresholdDb: -12, ratio: 8, attackMs: 10, releaseMs: 180, makeupDb: 6 },
	renderQuality: 'studio',
	playbackSeconds: 30,
	renderedSampleRate: 48000
}, base);
assert('all changed', multiChange.changedLabels.length === 5);
assert('labels include sound mode',  multiChange.changedLabels.includes('sound mode'));
assert('labels include focus',      multiChange.changedLabels.includes('focus'));
assert('labels include compression', multiChange.changedLabels.includes('compression'));

// ── advanceLoadState ──

console.log('\n📦 advanceLoadState');
const advOk = advanceLoadState(idle, { ok: true, requestedChannel: 'EHZ', actualChannel: 'EHZ' });
assert('advance success → loaded', advOk.state === 'loaded');

const advFallback = advanceLoadState(idle, { ok: true, requestedChannel: 'EHZ', actualChannel: 'SHZ' });
assert('advance fallback → fallback', advFallback.state === 'fallback');

const advFail = advanceLoadState(idle, { ok: false, error: 'timeout' });
assert('advance fail → failed', advFail.state === 'failed' && advFail.error === 'timeout');
