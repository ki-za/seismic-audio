...
import { prepareSamples, prepareSamplesChunked } from '../src/lib/domain/sonification';
...
async function main() {
	...
	const chunkedOutput = await prepareSamplesChunked(smallInput, 'soft', (pct) => progressLog.push(pct));
	...
	console.log('\n📦 Large array (> 500k)');
	const largeInput = Array.from({ length: 1_200_000 }, () => (Math.random() * 2 - 1));
	const syncLarge = prepareSamples(largeInput, 'raw');

	const largeProgress: number[] = [];
	const chunkedLarge = await prepareSamplesChunked(largeInput, 'raw', (pct) => largeProgress.push(pct));
	...
	for (let i = 0; i < syncLarge.length; i += 1) {
		if (Math.abs(syncLarge[i] - chunkedLarge[i]) > 1e-10) {
			largeMatch = false;
			console.log(`  mismatch at index ${i}: ${syncLarge[i]} vs ${chunkedLarge[i]}`);
			break;
		}
	}
	assert('identical output (large)', largeMatch);
	assert('multiple progress callbacks', largeProgress.length >= 3);
	assert('first progress ≈ 0.05', Math.abs(largeProgress[0] - 0.05) < 0.02, `got ${largeProgress[0].toFixed(4)}`);
	assert('final progress >= 0.99', largeProgress[largeProgress.length - 1] >= 0.99, `got ${largeProgress[largeProgress.length - 1].toFixed(4)}`);
	assert('progress monotonic', largeProgress.every((p, i) => i === 0 || p >= largeProgress[i - 1]));

	// ── Edge: empty array ──
	console.log('\n📦 Empty array');
	const emptyProgress: number[] = [];
	const emptyOutput = await prepareSamplesChunked([], 'soft', (pct) => emptyProgress.push(pct));
	...
	const exactInput = Array.from({ length: 500_000 }, () => (Math.random() * 2 - 1));
	const exactProgress: number[] = [];
	await prepareSamplesChunked(exactInput, 'soft', (pct) => exactProgress.push(pct));
